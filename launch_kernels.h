...
inline auto launch_paged_attn_v2(float *max_logits, float *exp_sums, T *out,
                                 T *tem_out, T *query, T *key_cache,
                                 T *value_cache, float *alibi_slopes,
                                 T *debug_out, U *block_tables, U *context_lens,
                                 uint32_t max_num_partitions, float sm_scale,
    ...
    cgh.parallel_for<kernel>(
        nd_range, [=](sycl::nd_item<3> item) SYCL_ESIMD_KERNEL {
          ...
          typename kernel::arguments_t args(
              ...
              reinterpret_cast<U *>(context_lens), sm_scale, num_seqs,
    ...
    cgh.parallel_for<kernel>(
        nd_range, [=](sycl::nd_item<3> item) SYCL_ESIMD_KERNEL {
          ...
          typename kernel::arguments_t args(
              ...
              reinterpret_cast<T *>(query), reinterpret_cast<T *>(key_cache),
              reinterpret_cast<T *>(value_cache),
              reinterpret_cast<float *>(alibi_slopes),
              reinterpret_cast<U *>(block_tables),
              reinterpret_cast<U *>(context_lens), sm_scale, num_seqs,
...
inline auto dispatch_paged_attention(
    ...
    T *out, T *tem_out, T *query, T *key_cache, T *value_cache,
    float *alibi_slopes, T *debug_out, U *block_tables, U *context_lens,
...
inline auto dispatch_paged_attention_loop(
    uint32_t head_size, uint32_t block_size, float *max_logits, float *exp_sums,
    T *out, T *tem_out, T *query, T *key_cache, T *value_cache,
    float *alibi_slopes, float *debug_out, U *block_tables, U *context_lens,
    uint32_t max_num_partitions, uint32_t num_queries_per_tokens,
    float sm_scale, uint32_t num_seqs, uint32_t num_heads,
    uint32_t num_kv_heads, uint32_t max_blocks_per_seq, float softcap) {
  if (head_size == 128 && block_size == 64) {
