...
#include "launch_kernels_fp8_loop.h"
...
std::vector<float> paged_attention_fp8_loop(
  ...
  using T = fp16;
  using U = uint32_t;
  constexpr auto fp8_t = gpu::xetla::fp8_format::E5M2;
  ...
  return dispatch_paged_attention_fp8_loop<T, U, fp8_t, arch_tag>(
      ...
      num_queries_per_tokens, sm_scale, fp8_scale, num_seqs, num_heads, num_kv_heads,
      max_blocks_per_seq, softcap);
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("run", &paged_attention_fp8_loop, "Paged Attention");
}
