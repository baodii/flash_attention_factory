...
template <typename policy, typename scalar_t, typename index_t, fp8_format fp8_format_,
          gpu_arch arch_tag>
class paged_attention_loop_kernel_fp8 {
public:
  using accum_t = float;
  ...
  struct arguments_t {
    ...
    inline arguments_t(scalar_t *out, accum_t *tem_out, scalar_t *query,
                       ...
                       accum_t sm_scale, float fp8_scale, uint32_t num_seqs, uint32_t num_heads,
...
#pragma unroll
  ...
  inline void compute_out(arguments_t &args, out_acc_tile_t &mat_acc_old,
    ...
    using value_scalar_tile_t = subgroup::tile_t<scalar_t, value_tile_desc_t>;
...
#pragma unroll(value_blocks_per_sg)
    for (uint32_t i = 0; i < value_blocks_per_sg; ++i) {
      ...
      SW_BARRIER();
      tile_mma::mma(cur_mat_acc_out, cur_mat_acc_out, mat_value_dequant, mat_exp_score);
      SW_BARRIER();
    }
